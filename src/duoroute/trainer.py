...
from duoroute.experiment_log import ExperimentRecord, finish_experiment, make_run_id, save_experiment
from duoroute.utils import load_yaml, project_root, set_seed
...
class GroupedQueryDataset(Dataset):
    def __init__(
        ...
        *,
        ...
        elif reward_target == "oracle_reward_exp":
            if data.oracle_reward_exp is None:
                raise ValueError(
                    "oracle_reward_exp missing in grouped.npz; run scripts/rebuild_oracle_rewards.py --modes exp"
                )
            oracle = data.oracle_reward_exp
        else:
            raise ValueError(f"Unknown reward_target: {reward_target}")
        ...
        self.performance = torch.from_numpy(data.performance.astype(np.float32))
    ...
    def __getitem__(self, index: int):
        return {
            ...
            "performance": self.performance[index],
        }
...
def _maybe_subsample(data: DuoRouteGroupedData, max_samples: Optional[int]) -> DuoRouteGroupedData:
    ...
    if n >= data.performance.shape[0]:
        return data
    oracle_exp = None
    if data.oracle_reward_exp is not None:
        oracle_exp = data.oracle_reward_exp[:n]
...
def train_duoroute(cfg: TrainConfig, *, config_path: str | None = None) -> dict:
    set_seed(cfg.seed)
    data_dir = Path(cfg.data_dir)
    pool_name = data_dir.name
    ...
    exp = ExperimentRecord(
        ...
        run_id=make_run_id("train", pool=pool_name, tag=tag.replace("DuoRoute-", "")),
        pool=pool_name,
        data_dir=str(data_dir.resolve()),
        config_path=config_path,
        hyperparameters=cfg.__dict__.copy(),
    )
    if config_path:
        try:
            exp.config = load_yaml(config_path).get("duoroute", {})
        except OSError:
            pass
    try:
        return _train_duoroute_impl(cfg, data_dir, out_dir, exp, tag)
    except Exception as e:
        finish_experiment(exp, status="failed", error=str(e), artifacts={"output_dir": str(out_dir.resolve())})
        save_experiment(exp, output_dir=out_dir)
        raise


def _train_duoroute_impl(
    cfg: TrainConfig,
    data_dir: Path,
    out_dir: Path,
    exp: ExperimentRecord,
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        val_oracle = (
            val_data.oracle_reward_exp
            if cfg.reward_target == "oracle_reward_exp" and val_data.oracle_reward_exp is not None
            else val_data.oracle_reward
        )
    ...
    test_oracle = (
        test_data.oracle_reward_exp
        if cfg.reward_target == "oracle_reward_exp" and test_data.oracle_reward_exp is not None
        else test_data.oracle_reward
    )
    ...
    best_val_regret = min((h["val_routing_regret"] for h in history), default=float("nan"))
    finish_experiment(
        exp,
        status="success",
        metrics={
            "test_query_only": test_metrics.to_dict(),
            "best_val_routing_regret": best_val_regret,
            "epochs": cfg.epochs,
        },
        artifacts={
            ...
            "test_pred_a": str((out_dir / "test_pred_a.npy").resolve()),
            "test_pred_b": str((out_dir / "test_pred_b.npy").resolve()),
        },
        extra={"history": history, "model_names": train_data.model_names},
    )
    save_experiment(exp, output_dir=out_dir)
    return results
