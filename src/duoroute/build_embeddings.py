...
def main() -> None:
    ...
    embed_exp = ExperimentRecord(
        experiment_type="embedding",
        run_id=make_run_id("embedding", pool=data_dir.name, tag=args.target),
        pool=data_dir.name,
        data_dir=str(data_dir.resolve()),
        config_path=args.config,
        hyperparameters={
            "target": args.target,
            "response_splits": args.response_splits,
            "only_datasets": args.only_datasets,
            "skip_empty_responses": args.skip_empty_responses,
            "resume": args.resume,
            "embedding_config": args.embedding_config or args.config,
        },
    )
    ...
    finish_experiment(
        ...
        artifacts={
            "data_dir": str(data_dir.resolve()),
            "embedding_meta": str((data_dir / "embedding_meta.json").resolve()),
        },
        extra={"response_stats": response_stats},
    )
    save_experiment(embed_exp, output_dir=data_dir / "experiments" / embed_exp.run_id)
...
if __name__ == "__main__":
    try:
        ...
        data_dir = Path(sys.argv[sys.argv.index("--data-dir") + 1]) if "--data-dir" in sys.argv else Path(".")
        fail = ExperimentRecord(
            experiment_type="embedding",
            run_id=make_run_id("embedding", pool=data_dir.name, tag="failed"),
            pool=data_dir.name,
            data_dir=str(data_dir),
            error=str(e),
        )
        finish_experiment(fail, status="failed", error=str(e))
        save_experiment(fail, output_dir=data_dir / "experiments" / fail.run_id)
        raise
